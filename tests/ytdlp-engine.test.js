...
describe("transcript args", () => {
  const TRANSCRIPT = {
    ...PATHS,
    url: "https://youtu.be/abc",
    outputDir: "/downloads",
    outputTemplate: "clip_transcript_68123.%(ext)s"
  }

  test("writes one subtitle track and no media at all", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
    ...
    expect(valueAfter(args, "--sub-langs")).toBe("en")
    expect(valueAfter(args, "--convert-subs")).toBe("srt")
    expect(args).toContain("--no-playlist")
    expect(args[args.length - 1]).toBe("https://youtu.be/abc")
  })

  // a human track must never quietly become a machine transcription of the
  // same language - the two read completely differently
  test("a human track asks for human subtitles only", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
      transcriptLanguage: "en",
      autoGenerated: false
  ...
  // the other direction is an upgrade rather than a surprise: yt-dlp prefers
  // the human track when a language is in both stores
  test("an automatic track accepts a human one if there is one", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
      transcriptLanguage: "en",
      autoGenerated: true
  ...
  test("txt is downloaded as srt - it is made out of one afterwards", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
      transcriptLanguage: "en",
  ...
  test("vtt is asked for as itself", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
      transcriptLanguage: "en",
    ...
    expect(valueAfter(args, "--convert-subs")).toBe("vtt")
  })

  test("a format we never offered falls back to srt", () => {
    for (const transcriptFormat of ["docx", "", null, undefined, 7]) {
      const args = buildArgs("transcript", {
        ...TRANSCRIPT,
        transcriptLanguage: "en",
        transcriptFormat
  ...
  /**
   * --sub-langs reads commas, globs and a leading "-" as list syntax, so an
   * unvetted tag could ask for every language at once - or, with the "-"
   * form, exclude the only one the user picked
   */
  test("a tag that is not a language code never reaches --sub-langs", () => {
    for (const transcriptLanguage of [
      "en,fr",
      "all",
      "-en",
      "en.*",
  ...
  test("the long caption tags youtube really serves are allowed through", () => {
    for (const code of ["en", "pt-BR", "zh-Hant-HK", "en-orig", "es-419"]) {
      const args = buildArgs("transcript", {
        ...TRANSCRIPT,
        transcriptLanguage: code
      })

      expect(valueAfter(args, "--sub-langs")).toBe(code)
    }
  })

  test("the output template still gets the naming guards", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
      transcriptLanguage: "en"
    })
    ...
    expect(valueAfter(args, "-P")).toBe("/downloads")
    expect(valueAfter(args, "-o")).toBe("clip_transcript_68123.%(ext)s")
    expect(args).toContain("--windows-filenames")
    expect(valueAfter(args, "--trim-filenames")).toBe("240")
  })

  // ffmpeg does the conversion, so the location has to be on the line
  test("ffmpeg is still located for the conversion", () => {
    const args = buildArgs("transcript", {
      ...TRANSCRIPT,
      transcriptLanguage: "en"
    })

    expect(valueAfter(args, "--ffmpeg-location")).toBe(PATHS.ffmpegPath)
  })
})
